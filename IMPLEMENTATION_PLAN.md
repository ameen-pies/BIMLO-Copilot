...
| 1.5 | **Cancel stops working after route picked** — backend keeps burning API credits after user cancels | P0 | `Backend/services/rag_engine.py` (_wrap, _chat, all LLM calls) | Two-layer fix: (1) `_wrap()` checks `_is_cancelled()` before each node + catches cancel RuntimeError mid-execution. (2) `_chat()` wrapper checks cancel before every `self.llm.chat()` call (9 calls). Intent classifier + graph_agent LLM calls still run but are fast; inter-node check catches them on next node. | `[x]` |
...
| # | Issue | Priority | Files | Fix | Status |
|---|-------|----------|-------|-----|--------|
| 2.1 | **Rate limit shows "Something went wrong"** — 429 response not handled, generic error displayed | P1 | `Frontend/src/components/AuthModal.tsx` | 429 detected, expiry stored in localStorage, persistent countdown (survives refresh), button disabled during countdown, amber warning box with "Try again in Xs". | `[x]` |
