...
| 1.4 | **Multi-doc RAG ignores second doc** — attached_doc_filenames processed but retrieval only returns chunks from one document | P0 | `Backend/services/rag_engine.py` retrieve_vector, `Backend/services/intent_classifier.py` post-process step 6 | Root cause: intent_classifier collapsed attached list to last file; retrieve_vector only searched that one file. Fix: retrieve_vector now checks `attached_doc_filenames` from state — when multiple files attached and no specific doc targeted, searches each file individually and merges results. Intent classifier step 6 only sets doc_scope for single-file attachments. | `[x]` |
